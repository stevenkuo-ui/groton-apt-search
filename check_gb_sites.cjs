...
async function main() {
  // Groton Townhomes available rentals (follow redirect manually)
  ...
  const ambPrices = amb.body.match(/\$\d{1,3}(,\d{3})*\/mo/g) || [];
  console.log('Ambrose prices:', [...new Set(ambPrices)].join(', '));
  
  // Look for 1995 and 2345 specifically
  const has1995 = amb.body.includes('1,995') || amb.body.includes('1995');
  const has2345 = amb.body.includes('2,345') || amb.body.includes('2345');
  console.log('Has 1,995:', has1995, 'Has 2,345:', has2345);
  ...
  // Check Groton Towers via grotonapartmenthomes.com/floorplans
  ...
  const gtTowerPrices = gtTower.body.match(/\$\d{1,3}(,\d{3})*\/mo/g) || [];
  console.log('GT Towers prices:', [...new Set(gtTowerPrices)].slice(0, 10).join(', '));
}
